...
class RepositoryPayroll
{
     ...
     * @return mixed
     */
    public function getSearchPaginated($criterion, $search, $status, $date, $type)
    {   
        $rg = $this->modelDAT->select(  
                            ...
                            ->join('users','users.id', "=", 'tickets.seller_id');
                ...
                if($date){
                    switch ($type) {
                        case 'day':
                            ...
                            break;
                        case 'week':
                            $rg->whereDate('day_tickets.game_date','>=',$date->startOfWeek())->whereDate('day_tickets.game_date','<=',$date->endOfWeek());
                            break;
                            
                        case 'month':
                            $rg->whereMonth('day_tickets.game_date', $date->month)->whereYear('day_tickets.game_date', $date->year);
                            break;
                        default:
                            $rg->whereDate('day_tickets.game_date', $date);
                            break;
                ...
                $Tickets = $rg->groupBy('users.phone','users.name','users.last_name')->whereNull('tickets.deleted_at')->orderBy('tickets.id', 'desc')->paginate(10);
